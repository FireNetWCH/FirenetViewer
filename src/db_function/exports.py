...
class pdf_worker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            print(f"Error przy exporcie PDF: {e}")
            logger.error(f"Error przy exporcie PDF: {e}")
class exel_worker(QObject):
        ...
        def run(self):
            try:  
                ...
                print(f"Error przy exporcie Exec: {e}")
                logger.error(f"Error przy exporcie Exec: {e}")
...
def generate_pdf(output_path, sender, receiver, date, subject, attachments, body,id_email,name_email_box,soft_text):
    ...
    pdfmetrics.registerFont(TTFont('DejaVu', get_resource_path('DejaVuSansCondensed.ttf')))
